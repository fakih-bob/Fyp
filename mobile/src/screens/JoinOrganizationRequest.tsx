...
export default function OrganizationRequestsScreen() {
  ...
  const fetchMyOrganizations = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        'http://192.168.10.157:8000/api/myorganizations', // Your API to fetch user's organizations
  ...
  const fetchRequests = async (orgId: number) => {
    ...
    try {
      const token = await AsyncStorage.getItem('token');
      const response = await axios.get(
        `http://192.168.10.157:8000/api/getRequestsForOrganization/${orgId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const acceptRequest = async (requestId: number) => {
    ...
    try {
      const token = await AsyncStorage.getItem('token');
      await axios.put(
        `http://192.168.10.157:8000/api/acceptRequest/${requestId}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const declineRequest = async (requestId: number) => {
    ...
    try {
      const token = await AsyncStorage.getItem('token');
      await axios.put(
        `http://192.168.10.157:8000/api/declineRequest/${requestId}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
