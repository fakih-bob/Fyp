  ...
  const fetchDepartmentDetails = async () => {
    try {
      ...
      const deptResponse = await axios.get(
        `http://192.168.10.157:8000/api/departments/${organizationId}`,
        {
      ...
      try {
        const usersResponse = await axios.get(
          `http://192.168.10.157:8000/api/department-users/${departmentId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        if (currentDept?.admin_id) {
          try {
            const adminResponse = await axios.get(
              `http://192.168.10.157:8000/api/user/${currentDept.admin_id}`,
              {
                headers: { Authorization: `Bearer ${token}` },
              }
            );
      ...
      try {
        const requestsResponse = await axios.get(
          `http://192.168.10.157:8000/api/maintenance-requests?department_id=${departmentId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        try {
          const altResponse = await axios.get(
            `http://192.168.10.157:8000/api/departments/${departmentId}/requests`,
            {
              headers: { Authorization: `Bearer ${token}` },
            }
          );
