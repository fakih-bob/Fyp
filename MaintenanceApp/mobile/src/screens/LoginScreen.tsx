...
const LoginScreen: React.FC = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Expecting: { status, token, data: { ...user } }
      ...
      // 🔒 Restriction check (supports boolean or numeric flags)
      const isRestricted = user?.is_restricted === true || user?.is_restricted === 1 || user?.is_restricted === '1';
      if (isRestricted) {
        // Make sure nothing is stored if blocked
        await AsyncStorage.removeItem('token');
        await AsyncStorage.removeItem('user');

        Alert.alert('Account Blocked', 'Your account is blocked. Please contact support.');
        return; // ⛔ stop here
      }
      ...
      // If backend returns 423 (Locked) or a specific message, show it
      const serverMsg =
