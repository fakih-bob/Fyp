...
type MyRequestsScreenNavigationProp = StackNavigationProp<
  RootStackParamList,
  'MyRequests'
>;
...
type MaintenanceRequest = {
  ...
  title: string;
  description: string;
  status: 'new' | 'pending' | 'approved' | 'in-progress' | 'done' | 'declined' | string;
  created_at: string;
  updated_at?: string;
  user?: UserLite;
...
/** 1x1 transparent PNG data URI used as final fallback */
const FALLBACK_DATA_URI =
  'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMB/Un8I4UAAAAASUVORK5CYII=';

/** Append cache-buster to avoid stale/partial cached responses */
const withBust = (url: string) => {
  const sep = url.includes('?') ? '&' : '?';
  return `${url}${sep}cb=${Date.now()}`;
};

/** Local cache path for a given (busted) URL */
const cachePathFor = (url: string) => `${FileSystem.cacheDirectory}img_${encodeURIComponent(url)}.bin`;

/** ---------------- PhotoThumb ----------------
 * Displays a small photo thumb. If direct load fails:
 *  1) downloads file (no headers),
 *  2) downloads with Authorization if token exists,
 *  3) falls back to transparent PNG.
 */
const PhotoThumb: React.FC<{
  ...
  const [uri, setUri] = useState(busted);
  const [stage, setStage] = useState<'remote' | 'dl-noauth' | 'dl-auth' | 'fallback'>('remote');

  const downloadThenShow = async (headers?: Record<string, string>) => {
    try {
      const target = cachePathFor(busted);
      const res = await FileSystem.downloadAsync(busted, target, { headers });
      console.log('[PhotoThumb] download', res.status, busted, '->', target);
      if (res.status >= 200 && res.status < 300) {
        setUri(res.uri); // show local file
        return true;
      }
    } catch (e) {
      console.warn('[PhotoThumb] download error', e);
    }
    return false;
  };

  const handleError = async () => {
    console.log('[PhotoThumb] onError at stage:', stage, 'url:', busted);
    if (stage === 'remote') {
      setStage('dl-noauth');
      const ok = await downloadThenShow(); // try public access
      if (ok) return;

      if (token) {
        setStage('dl-auth');
        const okAuth = await downloadThenShow({ Authorization: `Bearer ${token}` });
        if (okAuth) return;
      }

      setStage('fallback');
      setUri(FALLBACK_DATA_URI);
      return;
    }

    if (stage === 'dl-noauth' && token) {
      setStage('dl-auth');
      const okAuth = await downloadThenShow({ Authorization: `Bearer ${token}` });
      if (okAuth) return;
      setStage('fallback');
    ...
    // If dl-auth also failed (or nothing else to try)
    setStage('fallback');
    setUri(FALLBACK_DATA_URI);
  };
  ...
  return (
    ...
    >
      <Animated.Image
        source={{ uri }}
        style={{ width: '100%', height: '100%', transform: [{ scale: scaleAnim }] }}
        resizeMode="cover"
        onLoadStart={() => console.log('[PhotoThumb] load start', uri)}
        onError={handleError}
      />
    </View>
  );
...
export default function MyRequestsScreen() {
  ...
  const fetchAllRequests = async () => {
    setLoading(true);
    try {
      await Promise.all([fetchMyRequests(), fetchMyMaintenanceRequests()]);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const fetchMyMaintenanceRequests = async () => {
    try {
      ...
      const mapped: MaintenanceRequest[] = raw.map((r) => ({
        ...r,
        ...
        department: r.department
          ? { id: r.department.id, name: r.department.name, description: r.department.description }
          : undefined,
        assignee: r.assignee
          ? { id: r.assignee.id, name: r.assignee.name, email: r.assignee.email, role: r.assignee.role }
          : undefined,
        user: r.user ? { id: r.user.id, name: r.user.name, email: r.user.email, role: r.user.role } : undefined,
      }));
  ...
  const getStatusColor = (status: string) => {
    switch ((status || '').toLowerCase()) {
      case 'new':
        return theme.colors.adminBlue;
      case 'pending':
        return theme.colors.warning;
      case 'approved':
        return theme.colors.success;
      case 'in-progress':
        return theme.colors.maintenanceTeal;
      case 'done':
        return theme.colors.success;
      case 'declined':
  ...
  const renderMaintenanceRequest = ({ item }: { item: MaintenanceRequest; index: number }) => (
    <Animated.View
      style={[
        {
          opacity: fadeAnim,
      ...
      <Surface style={styles.requestCard} elevation={4}>
        <View style={styles.cardHeader}>
          ...
          <View style={styles.cardHeaderInfo}>
            <Text style={[styles.requestTitle, { color: theme.colors.charcoal }]}>{item.title}</Text>
            <Text style={[styles.requestDate, { color: theme.colors.slate }]}>{formatDate(item.created_at)}</Text>
          </View>
          <Chip
        ...
        {/* Photos gallery with resilient loading */}
        {item.photos && item.photos.length > 0 && (
          ...
          >
            {item.photos.map((ph) => (
              <PhotoThumb key={ph.id} url={ph.url} token={token} scaleAnim={scaleAnim} />
            ))}
          </ScrollView>
        )}
        ...
        <View style={styles.requestMeta}>
          {item.department && (
            <View style={styles.metaItem}>
              ...
              <Text style={[styles.metaText, { color: theme.colors.slate }]}>{item.department.name}</Text>
            </View>
          )}
          {item.assignee && (
            <View style={styles.metaItem}>
              <MaterialIcons name="person" size={16} color={theme.colors.adminBlue} />
              <Text style={[styles.metaText, { color: theme.colors.slate }]}>{item.assignee.name}</Text>
            </View>
          )}
          {item.photos && item.photos.length > 0 && (
            <View style={styles.metaItem}>
              <MaterialIcons name="photo" size={16} color={theme.colors.warning} />
              <Text style={[styles.metaText, { color: theme.colors.slate }]}>{item.photos.length} photo(s)</Text>
            </View>
          )}
        </View>
      </Surface>
  ...
  const renderOrganizationRequest = ({ item }: { item: OrganizationRequest; index: number }) => (
    <Animated.View
      style={[
        {
          opacity: fadeAnim,
    ...
    >
      <Surface style={styles.requestCard} elevation={4}>
        <View style={styles.cardHeader}>
          ...
          <View style={styles.cardHeaderInfo}>
            <Text style={[styles.requestTitle, { color: theme.colors.charcoal }]}>{item.organization.name}</Text>
            <Text style={[styles.requestDate, { color: theme.colors.slate }]}>{formatDate(item.created_at)}</Text>
          </View>
          <Chip
            icon={() => <MaterialIcons name={getStatusIcon(item.status)} size={16} color="white" />}
        ...
        {item.message && (
          <View style={styles.messageContainer}>
            ...
            <Text style={[styles.messageText, { color: theme.colors.slate }]}>{item.message}</Text>
          </View>
        )}
        ...
        <View style={styles.cardActions}>
          <Button
            ...
            disabled={cancelingId === item.id || item.status.toLowerCase() !== 'pending'}
            style={styles.cancelButton}
            buttonColor="transparent"
            textColor={item.status.toLowerCase() === 'pending' ? theme.colors.error : theme.colors.slate}
            icon="cancel"
            compact
          >
            {item.status.toLowerCase() === 'pending' ? 'Cancel' : 'Cannot Cancel'}
          </Button>
        </View>
      </Surface>
    </Animated.View>
  ...
  const renderTabSegments = () => (
    <Animated.View style={[styles.tabContainer, { opacity: fadeAnim, transform: [{ scale: scaleAnim }] }]}>
      <SegmentedButtons
        value={activeTab}
        ...
        buttons={[
          {
            ...
            label: 'Maintenance',
            icon: 'build',
            style: { backgroundColor: activeTab === 'maintenance' ? theme.colors.maintenanceTeal : 'transparent' },
          },
          {
            value: 'organization',
            label: 'Organizations',
            icon: 'business',
            style: { backgroundColor: activeTab === 'organization' ? theme.colors.userIndigo : 'transparent' },
          },
        ]}
  ...
  const renderEmptyState = () => (
    <Animated.View style={[styles.emptyContainer, { opacity: fadeAnim, transform: [{ scale: scaleAnim }] }]}>
      <Surface style={styles.emptyCard} elevation={2}>
        <LinearGradient
          colors={activeTab === 'maintenance' ? [theme.colors.maintenanceTeal, '#0B7D7A'] : [theme.colors.userIndigo, '#3F4BF0']}
          style={styles.emptyIconContainer}
        >
          <MaterialIcons name={activeTab === 'maintenance' ? 'build' : 'business'} size={48} color="white" />
        </LinearGradient>
        ...
        <Button
          ...
          onPress={() => {
            if (activeTab === 'maintenance') {
              ...
              Alert.alert('Join Organization', 'Would you like to request to join an organization?', [
                { text: 'Cancel', style: 'cancel' },
                { text: 'Join', onPress: () => Alert.alert('Info', 'Join organization feature will be implemented soon!') },
              ]);
            }
          }}
          style={[styles.emptyButton, { backgroundColor: activeTab === 'maintenance' ? theme.colors.maintenanceTeal : theme.colors.userIndigo }]}
        ...
        >
          ...
          {loading ? (
            <View style={styles.loadingContainer}>
              ...
              <Text style={[styles.loadingText, { color: theme.colors.slate }]}>Loading your requests...</Text>
            </View>
        ...
        {activeTab === 'organization' && (
          <Animated.View style={[styles.fabContainer, { opacity: fadeAnim, transform: [{ scale: scaleAnim }] }]}>
            <FAB
              ...
              icon="business"
              onPress={() =>
                Alert.alert('Join Organization', 'Would you like to request to join a new organization?', [
                  { text: 'Cancel', style: 'cancel' },
                  { text: 'Join', onPress: () => Alert.alert('Info', 'Join organization feature will be implemented soon!') },
                ])
              }
...
const styles = StyleSheet.create({
  ...
  statsCard: { borderRadius: 20, marginBottom: 20, overflow: 'hidden' },
  ...
  statsDivider: { width: 1, height: 40, backgroundColor: 'rgba(255, 255, 255, 0.3)', marginHorizontal: 16 },
  statsNumber: { fontSize: 20, fontWeight: '700', color: 'white', marginTop: 6, marginBottom: 2 },
  statsLabel: { fontSize: 11, fontWeight: '500', color: 'rgba(255, 255, 255, 0.9)', textAlign: 'center' },

  tabContainer: { marginBottom: 20 },
  segmentedButtons: { borderRadius: 16 },
  ...
  requestIcon: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
  cardHeaderInfo: { flex: 1, marginRight: 12 },
  requestTitle: { fontSize: 16, fontWeight: '700', marginBottom: 2, letterSpacing: 0.2 },
  requestDate: { fontSize: 12, opacity: 0.7 },
  statusChip: { alignSelf: 'flex-start' },

  requestDescription: { fontSize: 14, lineHeight: 18, marginBottom: 12, opacity: 0.8 },
  ...
  messageText: { fontSize: 12, flex: 1, lineHeight: 16 },
  cardActions: { flexDirection: 'row', justifyContent: 'flex-end', marginTop: 12 },
  cancelButton: { borderRadius: 8, borderWidth: 1 },
  ...
  emptyText: { fontSize: 14, textAlign: 'center', lineHeight: 20, opacity: 0.7, marginBottom: 8 },
  emptyIconContainer: { width: 80, height: 80, borderRadius: 40, justifyContent: 'center', alignItems: 'center', marginBottom: 8 },
  emptyButton: { borderRadius: 12, marginTop: 16, minWidth: 200 },
